...
from_hub_back_2_rollapp() {
    URAP_HASHED_DENOM=$(furyd q ibc-transfer denom-hash $IBC_PORT/$HUB_CHANNEL_NAME/urap | cut -d ' ' -f 2)
...
# ---------------------- moving FURY to rollapp and back ---------------------- #
from_hub_2_rollapp() {
    AMOUNT=$FURY_AMOUNT
    echo "sending $AMOUNT to $ROLLAPP_GENESIS_ADDR from $KEY_NAME_GENESIS"
...
from_rollapp_back_2_hub() {
    FURY_HASHED_DENOM=$(rollappd q ibc-transfer denom-hash $IBC_PORT/$ROLLAPP_CHANNEL_NAME/ufury | cut -d ' ' -f 2)
    AMOUNT="$TOKENS_AMOUNT"ibc/"$FURY_HASHED_DENOM"
    echo "sending $AMOUNT to $HUB_GENESIS_ADDR from $KEY_NAME_ROLLAPP"
...
query_test_accounts() {
    ...
    echo '# ------------------------------------ .. ------------------------------------ #'
    echo "Hub:"
    furyd q bank balances $HUB_GENESIS_ADDR --node "$SETTLEMENT_RPC_FOR_RELAYER"
